...
module.exports = (rawInput) => {
  ...
  const matchRegex = /^([0-9]*[.]?[0-9]+)?[\/]?([0-9]*[.]?[0-9]+)?([a-zA-Z]+)$/gm;
  const testRegex = /^([0-9./]*)?([a-zA-Z]+)$/gm;
  
  const test = testRegex.test(rawInput); // general format validation
  ...
  if(test) {
    ...
    const results = matchRegex.exec(rawInput);
    ...
    if(validNums && validUnit) {
      ...
      // TODO: do something for the bad input matches here?
      return {
        num: 'invalid number',
        unit: 'invalid unit'
      };
